...
def contourPlot(xvals,yvals,smooth=0,percentiles=[0.68,0.95,0.99],fillAlpha=None,colors=["red","green","blue"],xlabel=None,ylabel=None,xlim=None,ylim=None,filename=None,showPlot=False):
    ...
    if(type(xvals) is list):
        for ii in range(len(xvals)):
            ...
            if(fillAlpha is None):
                ...
                for ll,aa in zip(levels,fillAlpha):
                    plt.contourf(xx,yy,zz.T,levels=[ll],colors=[colors[ii]],alpha=aa)
...
def contourPlotAll(chains,lnprobs=None,inputPars=None,showMax=True,showPeakKDE=True,show68=True,smooth=0,percentiles=[0.68,0.95,0.99],fillAlpha=None,colors=["red","green","blue"],labels=None,limArr=None,figsize=(8,6),filename=None,showPlot=False):
    ...
    for row in range(nPars):
        for col in range(nPars):
            ...
            if(row == col):
                ...
                contourPlot(xarrs,yarrs,smooth=smooth,percentiles=percentiles,fillAlpha=fillAlpha,colors=contourColors,xlabel=xlabel,ylabel=ylabel)
                axarr[row,col].set_xlim(xlim)
...
def plotProfile(xarr,yarr,xlim=None,ylim=None,xscale="log",yscale="log",xlabel=None,ylabel=None,color="black",ls="-",lw=3,label=None,zorder=0):
    ...
    return plt.plot(xarr,yarr,color=color,ls=ls,label=label,lw=lw,zorder=zorder)
